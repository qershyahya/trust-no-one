"""Trust No One -- step 9: Which squares am I touching.   Run it:  python3 steps/step09.py"""
...
# every tile a box overlaps, usually two to six of them
def cells(rect):
    """Every tile this box overlaps -- usually two to six of them."""
    # the -1 means touching an edge exactly does not count as being inside
    for r in range(rect.top // TILE, (rect.bottom - 1) // TILE + 1):
        for c in range(rect.left // TILE, (rect.right - 1) // TILE + 1):
            ...
            ch = tile(c, r)
            if ch != " ":
                # hand them back one at a time as the loop asks, instead of building a list
                yield c, r, ch
