"""Trust No One -- step 11: Walls: the sideways pass.   Run it:  python3 steps/step11.py"""
...
def step(left, right):   # one frame of you
    ...
    # the level has two ends: max() keeps you off the left edge, min() off the right
    P["x"] = min(max(P["x"], 0.0), COLS * TILE - PW)

    P["y"] += P["vy"]   # and only now, vertically
