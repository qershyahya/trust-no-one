"""Trust No One -- step 13: Am I on the ground.   Run it:  python3 steps/step13.py"""
...
def die():   # back to the start, standing still
    ...
    # every one of your numbers, not just position. Leftover speed is a classic bug
    P.update(x=float(SPAWN[0]), y=float(SPAWN[1]), vx=0.0, vy=0.0, g=False)
...
def step(left, right):   # one frame of you
    ...
    # g for grounded. Move the box one pixel down and ask what is there. any() is True if at
    # least one answer is
...
def draw(scr):   # everything you see
    ...
    # proof on screen that the line above works. Step 19 takes it away again
