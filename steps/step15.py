"""Trust No One -- step 15: Coyote time.   Run it:  python3 steps/step15.py"""
...
# named after the cartoon coyote, who only falls once he looks down
...
# coyote frames left: how long ago you were last on the ground
coy = 0
...
def step(left, right, pressed=False, held=False):   # two new arguments
    ...
    # a countdown that refills every time you touch the ground, and ticks away while you are in
    # the air
    coy = COYOTE if P["g"] else coy - 1
    # pressed now, and grounded recently enough
    if pressed and coy > 0:
        # jump, and spend the credit so one ledge cannot give two jumps
        P["vy"], coy = JUMP, 0
