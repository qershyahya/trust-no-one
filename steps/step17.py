"""Trust No One -- step 17: Tap for a small hop.   Run it:  python3 steps/step17.py"""
...
# let go early while still rising and the rise is cut to this fraction
...
def step(left, right, pressed=False, held=False):   # two new arguments
    ...
    if buf > 0 and coy > 0:   # pressed recently AND grounded recently
        # jump, spend both credits, and remember that this rise is yours to cut
        P["vy"], buf, coy, P["jump"] = JUMP, 0, 0, True
    # let go early while still rising fast?
    if P["jump"] and not held and P["vy"] < JUMP * CUT:
        # cut the rise to 42% of full: a tap becomes a hop
        P["vy"], P["jump"] = JUMP * CUT, False    # let go early, hop short
    # once you are falling there is nothing left to cut
    if P["vy"] >= 0:
        # not mid-jump
        P["jump"] = False
