"""Trust No One -- step 18: Momentum.   Run it:  python3 steps/step18.py"""
...
# how fast you gain speed on the ground, how fast in the air, and how fast you lose it
ACC, AIR, FRIC = 0.55, 0.32, 0.72
...
def step(left, right, pressed=False, held=False):   # two new arguments
    ...
    # the speed you asked for -- which is no longer the speed you get
    want = (right - left) * SPD
    # 0.55 of steering on the ground, 0.32 in the air
    a = ACC if P["g"] else AIR
    if want:
        # move toward the speed you want by at most a. Never snap to it
        P["vx"] += max(-a, min(a, want - P["vx"]))
    else:
        # let go and you slide to a stop: quickly on the ground, slowly in the air
