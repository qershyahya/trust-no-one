...
LOOK = {"#": (150, 110, 70), "G": (90, 230, 190), "o": (240, 200, 60)}

# draw needs the font now, for the coin counter
def draw(scr, font):
    ...
    for r in range(ROWS):   # every row
        for c in range(cam // TILE, min(COLS, cam // TILE + VW // TILE + 2)):
            ...
            if ch == "o":
                # coins are circles, radius 9
                pygame.draw.circle(scr, LOOK[ch], box.center, 9)
            # the way out
            elif ch == "G":
    ...
    # render makes a small picture of the text and blit pastes it on. %d drops the number into
    # the string
...
def main():   # the whole game lives in here
    ...
    # any font the system has, at size 24
