"""Trust No One -- step 22: The wizard, and the curse.   Run it:  python3 steps/step22.py"""
...
# every tile is one letter: '#' brick  'o' coin  '^' spike  'W' wizard  'G' exit  'P' spawn
...
P = {}
# two flags. The entire twist hangs off the first
cursed = wizard = False
coins = coy = buf = 0
...
def step(left, right, pressed=False, held=False):   # two new arguments
    ...
    for c, rw, ch in cells(prect()):   # every square you are standing in, this frame
        ...
        # touched the wizard, and he is still here to touch
        elif ch == "W" and wizard:
            # he vanishes, and you are cursed. Nothing reads that flag yet
            wizard, cursed = False, True

LOOK = {"#": (150, 110, 70), "^": (170, 170, 180), "o": (240, 200, 60)}
    ...
    for r in range(ROWS):   # every row
        for c in range(cam // TILE, min(COLS, cam // TILE + VW // TILE + 2)):
            ...
            if ch == "o":
                ...
                # the wizard: purple, and narrower than his square
                pygame.draw.rect(scr, (160, 80, 220), box.inflate(-8, 0))
