...
# long enough to leave your hand
...
# every stone in the air right now
pebbles = []
...
def throw(tx, ty):
    ...
    # where it is, how fast, and its age in frames
...
def pebble_step():
    ...
    for pb in pebbles[:]:
        ...
        # what is written in the square it has reached
        ch = tile(c, r)
        # his letter stays in the level after he does, and it was eating stones
        if ch == "W" and not wizard:              # he has gone: his square is air
            continue
        # without this, the square you are standing on kills every throw
        if pb[4] < CLEAR:                         # still leaving your hand
            continue
        # something is written here, and it is not a coin you already took
        if ch != " " and (c, r) not in taken:
            pebbles.remove(pb)
