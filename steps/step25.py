"""Trust No One -- step 25: What the pebble reveals.   Run it:  python3 steps/step25.py"""
...
# half a second, and then it lies again
...
# squares a stone has struck, each with a countdown
hit = {}
...
def pebble_step():   # one frame for every pebble in the air
    ...
    # one frame off every countdown, wherever they are
    for cell in list(hit):                        # the truth fades on its own
        # a frame closer to lying again
        hit[cell] -= 1
        # its half second is up
        if hit[cell] <= 0:
            # and the square goes back to looking like whatever it likes
            del hit[cell]
    for pb in pebbles[:]:   # the [:] makes a copy
        ...
        if pb[4] < CLEAR:                         # still leaving your hand
            continue
        if ch != " " and (c, r) not in taken:   # something is written here
            for i in (-1, 0, 1):
                for j in (-1, 0, 1):
                    # the square and the ring around it, each with its own countdown
                    hit[(c + i, r + j)] = SHOWN
            pebbles.remove(pb)
...
# what each liar looks like once a stone has found it out: the same hue, duller. Assigned once
TRUTH = {"%": (108, 84, 66), "t": (146, 162, 148), "x": (206, 168, 96),
         "~": (50, 50, 66), "c": (126, 92, 58), "!": (66, 178, 158)}

# a darker version of any colour, for the outline
def edge(col):
    return tuple(max(0, v - 40) for v in col)

# the wobble: a sine that shrinks as the countdown runs out
def shaken(n):
