"""Trust No One -- step 26: Spikes that are trampolines.   Run it:  python3 steps/step26.py"""
...
# BOUNCE is stronger than JUMP: a trampoline throws you higher than you can jump
GRAV, SPD, JUMP, BOUNCE, MAXFALL = 0.35, 3.6, -9.2, -12.3, 12
...
# lies:   '%' hologram brick   't' spike that is a trampoline
...
coins = coy = buf = 0
taken, revealed, pebbles = set(), set(), []
...
def load():   # get the level ready to play
    ...
    taken.clear(); revealed.clear(); pebbles.clear()
...
def step(left, right, pressed=False, held=False):   # two new arguments
    ...
    for c, rw, ch in cells(prect()):   # every square you are standing in, this frame
        if ch == "^" or (ch == "t" and not cursed):
            return die()   # back to the start, and nothing else this frame
        # the spikes, once you can see them for what they are
        if ch == "t" and cursed:
            # fire upward, and forget you were grounded or mid-jump -- so the cut in step 17
            # cannot shorten a trampoline
...
LOOK = {"#": (150, 110, 70), "%": (150, 110, 70), "^": (170, 170, 180), "t": (170, 170, 180), "o": (240, 200, 60)}
# ponytail: a revealed lie keeps its own hue and goes darker/duller -- readable, never neon
TRUTH = {"%": (108, 84, 66), "t": (146, 162, 148)}
    ...
    for r in range(ROWS):   # every row
        for c in range(cam // TILE, min(COLS, cam // TILE + VW // TILE + 2)):
            ...
            if ch == "o":
                ...
                # the one line where a lie becomes visible
