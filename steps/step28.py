"""Trust No One -- step 28: Floor you cannot see.   Run it:  python3 steps/step28.py"""
...
# lies:   '%' hologram brick   't' spike that is a trampoline   'x' coin that kills   '~' floor that isn't drawn
...
def solid(ch):   # which letters stop you
    # the invisible floor is solid like any brick; it is only the drawing that leaves it out
    return ch in "#~" or (ch == "%" and not cursed)
...
LOOK = {"#": (150, 110, 70), "%": (150, 110, 70), "^": (170, 170, 180), "t": (170, 170, 180), "o": (240, 200, 60), "x": (240, 200, 60)}
# ponytail: a revealed lie keeps its own hue and goes darker/duller -- readable, never neon
TRUTH = {"%": (108, 84, 66), "t": (146, 162, 148), "x": (206, 168, 96), "~": (50, 50, 66)}
...
def draw(scr, font):   # draw needs the font now, for the coin counter
    ...
    for r in range(ROWS):   # every row
        for c in range(cam // TILE, min(COLS, cam // TILE + VW // TILE + 2)):
            ...
            # drawn only once a pebble has found it
            elif ch == "~":
                if seen:
