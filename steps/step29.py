"""Trust No One -- step 29: Bricks that crumble.   Run it:  python3 steps/step29.py"""
...
# 0.3 seconds of warning, half a second of nothing
...
# lies:   '%' hologram brick   't' spike that is a trampoline   'x' coin that kills   '~' floor that isn't drawn   'c' brick that crumbles
...
# squares that have dropped away: neither drawn nor solid
gone = set()
# every cracked brick you have stood on, and how far its clock has got
crack = {}
...
def reset():   # a whole fresh run
    ...
    global cursed, wizard, lives, over, coins
    cursed = False   # the curse: not yet
    wizard = True   # he is standing in the level until you touch
    coins = 0   # how many you have taken
    lives = LIVES   # how many you have left, right now
...
def cells(rect):   # every tile a box overlaps
    ...
    for r in range(rect.top // TILE, (rect.bottom - 1) // TILE + 1):   # the -1 means touching
        for c in range(rect.left // TILE, (rect.right - 1) // TILE + 1):
            ...
            # something is written here and it has not dropped away
...
def step(left, right, pressed=False, held=False):   # two new arguments
    global cursed, wizard, coins, frames, coy, buf
    ...
    # list() takes a copy, because the loop deletes from crack as it goes
    for cell in list(crack):                      # every cracked brick has a clock
        # one more frame on this brick's clock
        crack[cell] += 1
        # the warning is over
        if crack[cell] == CRACK:
            # into gone, which means neither drawn nor solid
            gone.add(cell)                        # the warning is over: it drops
        # its time away is up
        elif crack[cell] >= CRACK + AWAY:
            # you are standing in the square it wants to fill
            if prect().colliderect(pygame.Rect(cell[0] * TILE, cell[1] * TILE, TILE, TILE)):
                # hold the clock there and try again next frame, rather than building a brick
                # through you
                crack[cell] = CRACK + AWAY        # you are standing in its way: wait
            else:
                # the clock is thrown away and the brick is solid again
                del crack[cell]; gone.discard(cell)
    # and once started, stepping off will not save it
    if P["g"]:                                    # a brick you stand on starts its clock
        for c, rw, ch in cells(prect().move(0, 1)):
            # only start a clock that is not already running
            if ch == "c" and (c, rw) not in crack:
                # that exact square, from frame zero
                crack[(c, rw)] = 0
...
def pebble_step():   # one frame for every pebble in the air
    ...
    for pb in pebbles[:]:   # the [:] makes a copy
        ...
        if pb[4] < CLEAR:                         # still leaving your hand
            continue
        # something is written here, it has not broken away, and it is not a coin you already
        # took
        if ch != " " and (c, r) not in gone and (c, r) not in taken:
            for i in (-1, 0, 1):
                for j in (-1, 0, 1):
    ...
    for r in range(ROWS):   # every row
        for c in range(cam // TILE, min(COLS, cam // TILE + VW // TILE + 2)):
            ...
            if (c, r) in taken:   # a coin you took is not drawn
                continue
            # a square that has dropped away is not drawn
            if (c, r) in gone:
                continue
