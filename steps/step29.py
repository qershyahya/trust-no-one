...
# 26 frames is a little under half a second
...
def load():   # get the level ready to play
    ...
    # every level starts clean. Forgetting one of these is a real bug
...
def step(left, right, pressed=False, held=False):   # two new arguments
    ...
    if P["g"]:                                    # crumbling bricks only count while stood on
        for c, rw, ch in cells(prect().move(0, 1)):
            if ch == "c":
                # count the frames for that exact square. .get with a 0 means the first frame
                # needs no special case
                crumb[(c, rw)] = crumb.get((c, rw), 0) + 1
                # held long enough
                if crumb[(c, rw)] > CRUMB:
                    # into gone, and gone squares are neither drawn nor solid
                    gone.add((c, rw))
...
TRUTH = {"%": (108, 84, 66), "t": (146, 162, 148), "x": (206, 168, 96), "~": (50, 50, 66), "c": (126, 92, 58)}
    ...
    for r in range(ROWS):   # every row
        for c in range(cam // TILE, min(COLS, cam // TILE + VW // TILE + 2)):
            ...
            if ch == "c" and crumb.get((c, r)):
                # shake it two pixels, so you get a warning
