"""Trust No One -- step 30: The exit that lies.   Run it:  python3 steps/step30.py"""
    ...
    for r in range(ROWS):   # every row
        for c in range(cam // TILE, min(COLS, cam // TILE + VW // TILE + 2)):
            ...
            if ch in "G!":   # the way out, and the exit that lies
                ...
                # found out: the fake exit in its true colour
                if seen and ch == "!": col = TRUTH["!"]
