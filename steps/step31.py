"""Trust No One -- step 31: Dice.   Run it:  python3 steps/step31.py"""
...
LVL, COLS, ROWS, SPAWN, seed = [], 0, 0, (0, 0), 0
P = {}
cursed = wizard = False   # two flags
coins = frames = coy = buf = 0
taken, revealed, gone, pebbles, crumb = set(), set(), set(), [], {}

# one row of the level, and this run's dice
def roll(row, rng):
    """'?' and '&' pick a side per run, from this run's dice."""
    out = list(row)
    i = 0
    while i < len(out):
        # found a rollable square
        if out[i] in "?&":
            ch, j = out[i], i
            # find the whole run of them, e.g. ????
            while j < len(out) and out[j] == ch:
                j += 1
            # which of the two you can survive
            safe, other = ("t", "^") if ch == "?" else ("#", "%")
            # roll each square in the run
            span = [rng.choice((safe, other)) for _ in range(j - i)]
            # write the rolled squares back over the ?s
            out[i:j] = span
            i = j
        else:
            i += 1
...
def load():   # get the level ready to play
    ...
    # one seed gives the same level every time, which is how you re-open a bug
    rng = random.Random(seed * 977)
    LVL = [roll(r.ljust(COLS), rng) for r in L1]
...
def reset():   # a whole fresh run
    global cursed, wizard, coins, seed
    # a new seed per run, so no two runs are the same. 1 << 30 is just a big number
