"""Trust No One -- step 32: Dice that cannot trap you.   Run it:  python3 steps/step32.py"""
...
def roll(row, rng):   # one row of the level, and this run's dice
    ...
    while i < len(out):
        if out[i] in "?&":   # found a rollable square
            ...
            # every single one came up bad, so:
            if safe not in span:
                # force one back. Those two lines are the whole guarantee
...
def load():   # get the level ready to play
    ...
    rng = random.Random(seed * 977)   # one seed gives the same level every time
