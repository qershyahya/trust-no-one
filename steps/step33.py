...
# one name per level, for the bar along the top
...
# the last exit has been reached
won = False
    ...
    while True:   # the game loop
        ...
        for e in pygame.event.get():   # everything that happened since the last frame
            ...
            if e.type == pygame.KEYDOWN:
                # space starts a fresh run from the game over screen and from the win screen
                # alike
                if (over or won) and e.key == pygame.K_SPACE: reset()
