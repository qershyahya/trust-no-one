"""Trust No One -- step 34: The bar along the top.   Run it:  python3 steps/step34.py"""
...
# frames into minutes and seconds: 60 frames is a second, 3600 a minute
def clock_str(f):
    # %02d pads to two digits, so it reads 1:05 and not 1:5
...
def draw(scr, font, big):
    ...
    # which level, its time, coins, and the run's total
    hud = "%s   %s   coins %d   total %s" % (NAMES[level], clock_str(frames), coins, clock_str(total))
    ...
    # for the first couple of seconds of a level, show its name instead of the tip
    if frames < 140 and not won:
        tip = NAMES[level]
    if tip:
        t = big.render(tip, True, (255, 255, 255))
        # get_rect(center=...) centres the text without any arithmetic
...
def main():   # the whole game lives in here
    ...
    while True:   # the game loop
        ...
        # once you have won, stop simulating: the victory screen holds still
        if not won:
