...
# how hard the screen is bending right now: a jolt that fades a little every frame
warp = 0.0
# how far the air itself has travelled, added up frame by frame
...
def step(left, right, pressed=False, held=False):   # two new arguments
    ...
    # a jolt fades a little every frame -- before the run-is-over check, or the last death's
    # jolt would tear the game over screen for ever
...
def gusts(scr):
    ...
    global blown
    w = wind()
    # the old code used frames * w, which follows how fast the wind is changing, not which way
    # it blows
    ...
    for i in range(26):
        y = (i * 131) % VH
        # now a streak moves at 26 * w, which can never disagree with the shove you feel
