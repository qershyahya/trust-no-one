"""Trust No One -- step 38: Sound, out of arithmetic.   Run it:  python3 steps/step38.py"""
...
# name -> a Sound the mixer can play
SOUND = {}


# the whole soundtrack of effects on one page: each one a list of notes, from a pitch to a pitch
# over some seconds
TUNES = {                                        # every sound, as notes: (from, to, seconds)
    ...
    # the lowest note in the game, and the longest
...
# one table of levels, so no sound is a surprise: the music at 0.30 is the reference, a warning
# sits above it, a blip below
...
# build a note from nothing but arithmetic: square for a game sound, sine for music, noise for
# wind and stone
def note(f0, f1, secs, kind="square"):
    ...
    # 22050 samples a second, each one a whole number ("h" is a 16-bit signed integer)
    rate, out = 22050, array.array("h")
    # how many samples to write, and how far round the wave we are
    total, phase = int(rate * secs), 0.0
    for i in range(total):
        # walk round the circle a little further each sample; how far is the pitch
        phase += 2 * math.pi * (f0 + (f1 - f0) * i / total) / rate
        # a smooth wave, between -1 and 1
        wave = math.sin(phase)
        if kind == "square":
            # a square wave: all the way up, or all the way down
            wave = 1.0 if wave > 0 else -1.0
        # no pitch at all
        if kind == "noise":
            # a random number every sample is what a hiss is
            wave = random.uniform(-1.0, 1.0)
        # fade it out as it goes, or the end is a click, and 8000 is the volume
        out.append(int(wave * (1.0 - i / total) ** 3 * 8000))
    return out


# every sound in the game, built once when the game starts
def make_sounds():
    ...
    try:
        ...
        pygame.mixer.quit()
        # the same rate note() writes at, 16-bit, one channel, a small buffer so a jump sounds
        # on time. Get this wrong and every sound plays at the wrong pitch
        pygame.mixer.init(22050, -16, 1, 512)
    # a machine with no sound card should still play the game
    except pygame.error:
        return                                    # no sound card: play on in silence
    # every sound in the table
    for name, parts in TUNES.items():
        tune = array.array("h")
        for f0, f1, secs in parts:
            # the notes, one after another
            tune += note(f0, f1, secs)
        SOUND[name] = pygame.mixer.Sound(buffer=tune.tobytes())
    # a stone leaving your hand: a tenth of a second of noise
    SOUND["throw"] = pygame.mixer.Sound(buffer=note(0, 0, 0.12, "noise").tobytes())
    # two seconds of noise, played end to end for ever
    SOUND["wind"] = pygame.mixer.Sound(buffer=note(0, 0, 2.0, "noise").tobytes())
    for name, sound in SOUND.items():
        # every sound gets its level from the table, the made-up ones and the recordings alike
        sound.set_volume(VOLUME[name])           # so no effect shouts over another
    # its own channel, so it never interrupts a jump or a coin
    pygame.mixer.Channel(1).play(SOUND["wind"], loops=-1)   # always blowing; how loud is
    # silent until there is wind
    pygame.mixer.Channel(1).set_volume(0.0)                # for gusts() to say, every frame


# play one, if it exists
def beep(name):
    if name in SOUND:
        SOUND[name].play()
...
def step(left, right, pressed=False, held=False):   # two new arguments
    ...
    if buf > 0 and coy > 0:   # pressed recently AND grounded recently
        ...
        # a note that slides up, which is what makes it sound like a jump
        beep("jump")
...
def gusts(scr):   # the wind
    ...
    # the wind you can hear: loudness set from wind() itself, every frame
    if "wind" in SOUND:                           # the hiss follows the gust
        # the wind's own level, scaled by how hard it is blowing against the strongest gust in
        # the game
...
def main():   # the whole game lives in here
    ...
    # build the sounds before the first frame
    make_sounds()
