"""Trust No One -- step 39: A picture instead of a colour.   Run it:  python3 steps/step39.py"""
...
# the art folder, found from this file rather than from wherever you happened to be standing
# when you ran it
ART = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "art")
# one picture per letter of the level, filled in by art()
PIC = {}


# open one file from the art folder
def picture(name):
    """Open a picture. convert_alpha() re-packs it the way the screen wants,
    which makes every later blit far quicker."""
    # convert_alpha keeps the see-through parts and re-packs it the way the screen wants
    return pygame.image.load(os.path.join(ART, name)).convert_alpha()

# every picture, opened once, before the first frame
def art():
    """Every picture, opened once. Doing this inside draw() would open the same
    files sixty times a second."""
    # one file with every tile in it, in a grid
    sheet = picture("tiles.png")
    # subsurface is a window onto the same pixels -- no copying -- so .copy() takes a real one
    cell = lambda c, r: sheet.subsurface((c * TILE, r * TILE, TILE, TILE)).copy()
    # column 5, row 1 of the tile sheet
    ...
    for r in range(ROWS):   # every row
        for c in range(cam // TILE, min(COLS, cam // TILE + VW // TILE + 2)):
            ...
            if ch == "~" and not seen:   # floor you cannot see stays unseen until
                continue
            # a picture, if there is one for this letter yet
            pic = PIC.get(ch)
            # a picture if there is one for this letter, the old shape if there is not
            if pic:
                # blit copies one surface onto another. This is how everything is drawn from
                # here on
                scr.blit(pic, box)
                continue
