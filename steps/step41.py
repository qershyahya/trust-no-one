"""Trust No One -- step 41: Cutting a strip into frames.   Run it:  python3 steps/step41.py"""
...
# which way she points. Set from her speed, so she keeps facing the way she is sliding
face = 1                                              # 1 right, -1 left
COIN = []   # the six pictures of a spinning coin
# her animations by name, each a list of frames
PLAYER = {}
...
# a strip of n frames, side by side, cut into a list
def cut(sheet, n):
    ...
    # every frame is the same width, so the width of one is the whole divided by how many
    wide, out = sheet.get_width() // n, []
    for i in range(n):
        frame = sheet.subsurface((i * wide, 0, wide, sheet.get_height()))
        # the smallest rectangle holding every pixel that is actually drawn
        box = frame.get_bounding_rect()
        # trimmed, so a 150-wide frame with a small character in it stops being mostly air
        out.append(frame.subsurface(box).copy() if box.width else frame.copy())
    return out


# every frame, ready to face either way, built once at the start
def facing(frames):
    ...
    second."""
    # each frame twice: as drawn, and mirrored
    return [{1: f, -1: pygame.transform.flip(f, True, False)}
            for f in frames]
...
def art():   # every picture
    ...
    # eight frames of standing still
    PLAYER["idle"] = facing(cut(picture("player_idle.png"), 8))
...
# which frame of her to draw right now. Every later step adds a rule above the last line
def pose():
    """The frame of her to draw right now: the newest rule that applies wins."""
    # standing still
    seq = PLAYER["idle"]
    # one picture every six frames: ten a second
    return seq[(beat // 6) % len(seq)]
...
def draw(scr, font, big):   # two fonts now
    ...
    # the frame of her to draw, facing the way she points
    who = pose()[face]
