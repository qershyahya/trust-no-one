"""Trust No One -- step 42: Run, jump, fall.   Run it:  python3 steps/step42.py"""
...
def facing(frames):   # every frame
    ...
    second."""
    return [{1: f, -1: pygame.transform.flip(f, True, False)}   # each frame twice
            for f in frames]
...
def step(left, right, pressed=False, held=False):   # two new arguments
    ...
    # 0.4 rather than 0, or you would flicker while sliding to a stop
...
def pose():   # which frame of her to draw right now
    ...
    if not P["g"]:
        # going up or coming down: two different pictures
