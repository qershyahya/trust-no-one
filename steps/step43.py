"""Trust No One -- step 43: A rim, so you can be seen.   Run it:  python3 steps/step43.py"""
...
# a one-pixel outline, so she does not vanish into the night
def rimmed(pic, colour=(226, 236, 255)):
    ...
    # a mask is which pixels are drawn at all; to_surface paints that shape one flat colour
    edge = pygame.mask.from_surface(pic).to_surface(setcolor=colour,
                                                    unsetcolor=(0, 0, 0, 0))
    for dx, dy in ((0, 0), (1, 0), (2, 0), (0, 1), (2, 1), (0, 2), (1, 2), (2, 2)):
        # eight offsets: four sides and four corners
        out.blit(edge, (dx, dy))    # the shape, eight times, one pixel out each way
    # in the middle of the two-pixel border the eight copies made
...
def facing(frames):   # every frame
    ...
    second."""
    # and now each with its rim
    return [{1: rimmed(f), -1: rimmed(pygame.transform.flip(f, True, False))}
            for f in frames]
