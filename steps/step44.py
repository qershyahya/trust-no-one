"""Trust No One -- step 44: The sky, and the graveyard.   Run it:  python3 steps/step44.py"""
...
# the two background layers, far and near
SKY = []
...
def draw(scr, font, big):   # two fonts now
    ...
    # one pixel out of the picture, so the sky above it matches exactly
    scr.fill(SKY[0].get_at((4, 4)))               # the night, taken from the art
    # the far layer divided by 4, the near one by 2
    for layer, slower in ((SKY[0], 4), (SKY[1], 2)):
        ...
        for x in range(-wide, VW + wide, wide):
            ...
            # dividing the camera is the whole of parallax: move less, look further away
