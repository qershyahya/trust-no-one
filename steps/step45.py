"""Trust No One -- step 45: The truth, as a picture.   Run it:  python3 steps/step45.py"""
...
# what each liar really is, as a picture
TRUE = {}
# eight frames of flame: a coin that kills, once a stone has found it
FIRE = []
...
# scale to a height, keeping its shape
def tall(pic, height):
...
# trim a picture, scale it to one square, and stand it on the floor of that square
def fit(pic):
...
def art():   # every picture
    ...
    # eight frames of it, so the fire moves
...
def step(left, right, pressed=False, held=False):   # two new arguments
    global cursed, wizard, won, coins, frames, coy, buf, warp, face
...
def draw(scr, font, big):   # two fonts now
    ...
    for r in range(ROWS):   # every row
        for c in range(cam // TILE, min(COLS, cam // TILE + VW // TILE + 2)):
            ...
            # the same countdown as before; only what it draws has changed
            if seen and ch in TRUE:                  # for half a second, the truth
                pic = TRUE[ch]
                # some entries are one picture and some are a list of them
