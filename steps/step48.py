"""Trust No One -- step 48: A throw that looks like one.   Run it:  python3 steps/step48.py"""
...
# a bit over a quarter of a second
...
# frames left of the throw animation; while it is above zero it wins over every other pose
throwing = 0
...
# the two near-whites the artist used for the arc across the throw frame, so they can be rubbed
# out
SWOOSH = {(240, 240, 240), (214, 221, 225)}    # the two colours of the sword arc


# the artist drew a white arc across the throw frame; this rubs it out
def de_swoosh(frames):
    ...
    for frame in frames:
        ...
        # forty near-white pixels is a highlight, a thousand is the arc
        if white > 200:
...
def throw(tx, ty):   # tx, ty is where you clicked, in world pixels
    ...
    # and the animation runs itself down in step()
    throwing = THROW
...
def pose():   # which frame of her to draw right now
    ...
    # the throw animation wins over standing, running and falling
