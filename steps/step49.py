"""Trust No One -- step 49: Dust.   Run it:  python3 steps/step49.py"""
...
# eight frames of a plume
SMOKE = []
# every plume in the air: where, and how old
PUFFS = []
...
# a plume of smoke: where it is, and how old
def puff(x, y):
    """A plume of smoke, at a point in the world."""
    PUFFS.append([x, y, 0])


# draw every plume and age it. Gone in half a second
def smoke(scr, cam):
    """Draw every plume and age it. They drift up and are gone in half a second."""
    # a copy of the list, because the loop deletes from it
    for p in PUFFS[:]:
        p[2] += 1
        i = p[2] // 4
        if i >= len(SMOKE):
...
def pebble_step():   # one frame for every pebble in the air
    ...
    for pb in pebbles[:]:   # the [:] makes a copy
        ...
        if ch != " " and (c, r) not in gone and (c, r) not in taken:   # something is written
            ...
            beep("knock")
            # a little below the stone, so it comes off the surface
