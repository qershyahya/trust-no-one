"""Trust No One -- step 51: The keys, on the screen.   Run it:  python3 steps/step51.py"""
...
# the arrow keys, space and the mouse, as pictures for the corner of the screen
KEYS = {}
...
def art():   # every picture
    ...
    # every key on a keyboard, as pixel art, in one grid
    keys = picture("keys.png")
    # the same subsurface trick as the tile sheet, then doubled in size
...
def draw(scr, font, big):   # two fonts now
    ...
    # the keys that do something, and only those: a control you draw is a promise
    for i, k in enumerate(("left", "right", "space", "click")):
