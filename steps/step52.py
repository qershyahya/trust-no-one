"""Trust No One -- step 52: The wizard, awake.   Run it:  python3 steps/step52.py"""
...
# the flinch lasts 16 frames; the spell takes 40, and the curse lands when it is done
...
# frames left of his flinch, and of his spell
flash = casting = 0
...
KEYS = {}   # the arrow keys
# him standing, facing right and facing left
WIZ, WIZ_L = [], []
# him flinching, both ways
HURT, HURT_L = [], []
# him taking your eyes, facing right and facing left
CAST, CAST_L = [], []
    ...
    for r in range(ROWS):   # every row
        for c in range(cam // TILE, min(COLS, cam // TILE + VW // TILE + 2)):
            ...
            if ch == "W":
                ...
                # are you to his left? Then he is drawn from the mirrored list
                at_you = P["x"] + PW / 2 < box.centerx + cam
                # one counter per animation, and a plain if to choose
                if casting > 0:                      # taking your eyes
                    seq = CAST_L if at_you else CAST
                    # the counter, spread across however many frames the animation has
