"""Trust No One -- step 54: A conversation.   Run it:  python3 steps/step54.py"""
...
# the conversation, in order. The last line is his, and it starts the spell
...
# one word for what the opening is doing right now
scene, wiz_at = "title", None                  # title -> walk -> hit -> talk -> cast -> None
# counts up with every press of space
...
def scene_step():   # one frame of the opening
    ...
    if scene == "walk":
        ...
        if throwing == THROW // 2:
            ...
            # the swing is over: now he speaks
            scene = "talk"
...
def advance():   # space or a click
    ...
    if scene == "title":
        ...
        beep("talk")
        if said >= len(TALK):
            # his last line starts the spell; the curse lands when the spell finishes
            scene, casting = "cast", CURSE          # he raises his hands
            # one long slide, all the way down
            beep("curse")


# a speech bubble, measured from the text it holds
def bubble(scr, font, text, cx, top, colour):
    ...
    # the text's own rectangle, grown by the padding
    box = t.get_rect(centerx=cx, top=top).inflate(26, 16)
    # clamp_ip shoves it back inside the screen if it would hang off the edge
    ...
    # the tail points at the speaker, but never off the end of its own bubble
