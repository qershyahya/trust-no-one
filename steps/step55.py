"""Trust No One -- step 55: Levels you can edit.   Run it:  python3 steps/step55.py"""
...
# the text file the levels are read from, next to the game
LEVELS_TXT = os.path.join(os.path.dirname(ART), "levels.txt")
# the top of levels.txt: what every letter means, written into the file so it explains itself
LEGEND = [
...
# write the game's own levels out, once, so there is something to edit
def dump_levels():
    ...
    for name, rows in zip(NAMES, LEVELS):
        ...
        # the bars hold the width open: an editor that trims trailing spaces cannot narrow a
        # level
...
# a name in brackets, then its rows, each inside a pair of bars
def read_levels():
...
# take the levels from the file, if it makes sense
def load_levels():
    ...
    # a half-typed edit must not break the game you are playing
