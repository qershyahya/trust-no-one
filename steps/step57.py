"""Trust No One -- step 57: Four bars under all of it.   Run it:  python3 steps/step57.py"""
...
scene, wiz_at = "title", None                  # title -> walk -> hit -> talk -> cast -> None
said = 0                                       # how far through TALK we are


def find_wizard():   # which square he is standing in
...
def scene_step():   # one frame of the opening
...
def advance():   # space or a click
    """Space or a click. Only the parts you read wait for you."""
    global scene, wiz_at, said, casting
    if scene == "title":
        wiz_at = find_wizard()
...
def bubble(scr, font, text, cx, top, colour):   # a speech bubble
    """One speech bubble, with a tail pointing down at whoever said it."""
    t = font.render(text, True, (240, 236, 228))
    box = t.get_rect(centerx=cx, top=top).inflate(26, 16)   # the text's own rectangle
    box.clamp_ip(pygame.Rect(8, 8, VW - 16, VH - 16))   # clamp_ip shoves it back inside
    ...
    tx = max(box.left + 14, min(cx, box.right - 14))   # the tail points at the speaker
    pygame.draw.polygon(scr, colour, [(tx - 9, box.bottom - 2), (tx + 9, box.bottom - 2),
                                      (tx, box.bottom + 12)])

LEVELS_TXT = os.path.join(os.path.dirname(ART), "levels.txt")   # the text file the levels are
LEGEND = [   # the top of levels.txt
...
def dump_levels():   # write the game's own levels out, once
    ...
    for name, rows in zip(NAMES, LEVELS):
        ...
        out += ["|%s|" % r.ljust(wide) for r in rows]   # the bars hold the width open
        out.append("")
    with open(LEVELS_TXT, "w") as f:
        f.write("\n".join(out))


def read_levels():   # a name in brackets
...
def load_levels():   # take the levels from the file
    ...
    if not levels or bad:   # a half-typed edit must not break the game
...
RECORDED = {"jump": "jump.wav", "coin": "coin.wav", "die": "hurt.wav",   # six recordings
...
def load_recorded():   # six wav files from a free pack
...
# a tune is notes one after another, and you already have a function that makes a note
def music():
    ...
    for chord in bars:
        for i in range(8):
            # doubling a pitch is exactly one octave
            f = chord[i % 3] * (2 if i >= 4 else 1)   # the top half of the bar, an octave up
            # half a beat each, joined end to end; sine, so it sounds like music rather than a
            # game
...
def reset():   # a whole fresh run
    ...
    global cursed, wizard, lives, over, coins, seed, won, scene, flash, casting, throwing, said
...
def main():   # the whole game lives in here
    ...
    if SOUND:                                 # four bars, for ever, underneath
        # loops=-1 means for ever, and its own channel means it never interrupts a jump
